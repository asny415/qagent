...
export class AIAgent {
  ...
  async task(task: string, cb: ProgressCB) {
    ...
    this.msgBuffer = [
      {
        role: "user",
        content: `你是一个聪明的agent，会用用户的语言帮助查找想要的信息。你只允许利用下面的这些 python 函数扩展自己的能力, 如果你决定调用任何一个function(s), 你需要使用 \`\`\`tool_code\`\`\` 包裹它们，请尽可能使用简洁有效的单行单个函数调用. 函数的响应将会被包裹在 \`\`\`tool_output\`\`\` 中并且你应该假设用户看不到其中的内容，你可以利用函数响应的内容进行更多次函数调用或者直接生成对用户有帮助且友善的响应. 在通过 \`\`\`tool_call\`\`\` 进行函数调用时，请逐步解释为什么你需要这样做.
...
请你分析需要几个步骤来完成用户的任务，然后一步一步执行，请确保每一步都成功执行，并且在做每一步之前解释你在做的事情
`,
      },
    ];
